...
var score_col_bt  = document.getElementById("score_col_bt")
...
function clickOE() {
  ...
  $('#canvas').append(table_responsive);
...
function Start_Chart1(index) {
    ...
    call_oeall2(2, 1, 0, ti_1, un_1, index);
...
function histogram_single_measure_values(j,colors) {
...
function call_oeall2(_sort, _norm, _sortarea, _title, _units, index) { 
	...
	topcard.innerHTML = project[0].pname + " | " + 'Data View';
...
function Avg_Templates(_array01, _array02) {
  ...
  for (i = 0; i < _array01.length; i++) {
    ...
    var final_avg = Math.round(array_sum.reduce(function(a, b) {
      return a + b
    }) / _array01[i].length);
...
function gr_on(x) {
  ...
  if (gr == 1) {
    ...
    for (i = 0; i < array_temp.length; i++) {
      ...
      for (j = 0; j < drill_id.length; j++) {
        if (drill_id[j].temp == array_temp[i]) {
          drcat.data.push([drill_id[j].name, drill_id[j].oe_co.reduce(function(a, b) {
            return a + b
          })]);
        }
      }
    ...
    for (i = 0; i < array_temp.length; i++) {
      ...
      for (j = 0; j < drill_id.length; j++) {
        if (drill_id[j].temp == array_temp[i]) {
          drcat.data.push([drill_id[j].name, drill_id[j].oe_he.reduce(function(a, b) {
            return a + b
          })]);
        }
      }
    ...
    for (i = 0; i < array_temp.length; i++) {
      ...
      for (j = 0; j < drill_id.length; j++) {
        if (drill_id[j].temp == array_temp[i]) {
          drcat.data.push([drill_id[j].name, drill_id[j].oe_el.reduce(function(a, b) {
            return a + b
          })]);
        }
      }
    ...
    for (i = 0; i < array_temp.length; i++) {
      ...
      for (j = 0; j < drill_id.length; j++) {
        if (drill_id[j].temp == array_temp[i]) {
          drcat.data.push([drill_id[j].name, drill_id[j].oe_eq.reduce(function(a, b) {
            return a + b
          })]);
        }
      }
    ...
    for (i = 0; i < array_temp.length; i++) {
      ...
      for (j = 0; j < drill_id.length; j++) {
        if (drill_id[j].temp == array_temp[i]) {
          drcat.data.push([drill_id[j].name, drill_id[j].oe_co_norm.reduce(function(a, b) {
            return a + b
          })]);
        }
      }
    ...
    for (i = 0; i < array_temp.length; i++) {
      ...
      for (j = 0; j < drill_id.length; j++) {
        if (drill_id[j].temp == array_temp[i]) {
          drcat.data.push([drill_id[j].name, drill_id[j].oe_he_norm.reduce(function(a, b) {
            return a + b
          })]);
        }
      }
    ...
    for (i = 0; i < array_temp.length; i++) {
      ...
      for (j = 0; j < drill_id.length; j++) {
        if (drill_id[j].temp == array_temp[i]) {
          drcat.data.push([drill_id[j].name, drill_id[j].oe_el_norm.reduce(function(a, b) {
            return a + b
          })]);
        }
      }
    ...
    for (i = 0; i < array_temp.length; i++) {
      ...
      for (j = 0; j < drill_id.length; j++) {
        if (drill_id[j].temp == array_temp[i]) {
          drcat.data.push([drill_id[j].name, drill_id[j].oe_eq_norm.reduce(function(a, b) {
            return a + b
          })]);
        }
      }
...
function temp_columns(temp_title,temp_units) {
...
function Start_Chart4() {
  ...
  $(".box").onchange = function(){updatemap();};
...
function Start_Home(index) {
  ...
  //header.className = "span12";
  //$('#canvas').append(header);

  //header.id = "header"
  //header.innerHTML = header.innerHTML + project[0].plocation;
  ...
  $('#topcard').fadeIn(500);
	...
	banner.appendChild(unitDisplay);
		...
		//unitDisplay.class = "col-lg-3 col-md-3 col-sm-4 col-xs-4"	
	//unitDisplay.innerHTML = '<button id="sortButton">Sort</button>'
...
function BuildMap(data,title,units){
	building_geometries = [];
  	building_data = [];
...
function BuildBar(data,title,color,units,sort){
  ...
  for (i = 0; i < buildings.length; i++) {
    ...
    blddata.push(buildings[i].bname);
    blddata.push(buildings[i][data]);
	...
	if (sort == "yes") {
	building_data.sort(function(a, b) {
    return a[1] - b[1]
	});
	}
  ...
  $('#scoremap').highcharts({
    ...
    tooltip: {
      formatter: function() {
        return building_data[this.x][0] +'</b> has a value of <b>' + this.y + units;
    	}
    },
    plotOptions: {
      column: {
...
function BuildChart(type,data,title,color,units,sort){
    ...
    if (data == 'da' || data == 'wa'){
      ...
      document.getElementById("temp_Map").className = "btn btn-default active";
		  ...
		  BuildMap(data,title,units); 
      ToggleOffPanel();
      document.getElementById("temp_Map").className = "btn btn-default active";
    }
	}
	if (type == 'Bar'){
		BuildBar(data,title,color,units,sort); 
    ToggleOffPanel();
...
function ToggleOffPanel() {
document.getElementById("bld_home").className = "btn btn-default";
document.getElementById("temp_Map").className = "btn btn-default";
document.getElementById("bld_Data").className = "btn btn-default";
document.getElementById("scatter_bt").className = "btn btn-default";
}

scatterx = "oe_norm" 
scattery = "da"

function Scatter(xMetric,yMetric){
    ...
    for (i= 0; i < buildings.length; i++){
    	allTemps.push(buildings[i].temp);


   }

   colors = ['rgba(237,30,121,0.7)', 'rgba(0,169,157,0.7)', 'rgba(16,125,255,0.7)', 'rgba(255,190,32,0.7)', 'rgba(140,140,140,0.7)', 'rgba(237,30,121,0.7)', 'rgba(0,169,157,0.7)', 'rgba(16,125,255,0.7)', 'rgba(255,190,32,0.6)', 'rgba(140,140,140,0.6)', 'rgba(237,30,121,0.3)', 'rgba(0,169,157,0.3)', 'rgba(16,125,255,0.3)', 'rgba(255,190,32,0.3)', 'rgba(140,140,140,0.3)']

   tempNames = allTemps.filter(function(item, i, ar){ return ar.indexOf(item) === i; });
   ...
   Temps = [];
    ...
    for (i= 0; i < tempNames.length; i++){
    	...
    	var ArrayData = [];
    	for (j= 0; j < buildings.length; j++){
    		if (buildings[j].temp == tempNames[i]){
    			var data = [];
    			data.push(buildings[j][xMetric]);
    			data.push(buildings[j][yMetric]);
    			data.push(buildings[j].bname);
    			ArrayData.push(data);
    		}
    	}
    	a.data = ArrayData;
  ...
  $('#scoremap').highcharts({
    ...
    plotOptions: {
        scatter: {
            ...
            tooltip: {
                headerFormat: '<b>{Temps.[data][2]}</b><br>',
...
function ScatterButtonX(title){
	scatterx = title
	Scatter(scatterx,scattery);
}

function ScatterButtonY(title){
	scattery = title
	Scatter(scatterx,scattery);
}
